...
func FacebookLoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	url := fmt.Sprintf(
		"https://www.facebook.com/v10.0/dialog/oauth?client_id=%s&redirect_uri=%s&state=%s&scope=email&response_type=code",
		readenv.FacebookClientID,
		url.QueryEscape("http://localhost:8080/facebook/callback"),
		state,
	)
	http.Redirect(w, r, url, http.StatusTemporaryRedirect)
}

func FacebookRegisterHandler(w http.ResponseWriter, r *http.Request) {
...
func FacebookCallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	token, err := requests.GetFacebookAccessToken(code)
	if err != nil {
	...
	user, err := requests.GetFacebookUser(token)
	if err != nil {
	...
	if err == sql.ErrNoRows {
		_, err = db.Exec("INSERT INTO USERS (Email, UserName, Password, Role, session_token, AuthProvider) VALUES (?, ?, ?, ?, ?, ?)", user.Email, user.Name, "", "User", "", "facebook")
		if err != nil {
